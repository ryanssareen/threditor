...
- **`/ce:review` cross-reviewer convergence.** First pass: 9 parallel reviewers; the 2 P1 findings (GPU leak + texture race) came from 6-of-9 and 5-of-9 agreement respectively. Second pass (after the two visual-bug fixes): 8 reviewers; no P1s, all prior findings confirmed fixed at HEAD. Single-reviewer findings were mostly dismissed after inspection; convergence was the strongest signal for severity.
- **Split commits per logical change** (migration → font → lib/three → EditorCanvas → lockfile → review-fixes → visual-fixes → second-review-fixes). Clean bisect path if anything regresses.
- **Human visual verification caught two bugs the code review missed.** `transparent={true}` on base meshes (ghostly bleed-through) and the upper-torso camera crop. Both were pattern-level correctness issues — everything compiled, typechecked, passed lint and build — but only a rendered-pixel comparison surfaced them. Static verification is necessary, not sufficient.

### What didn't

- **ChatGPT's round-4 camera spec framed an upper-torso shot, not full-body.** Spec gave position `(0, 1.4, 3.2)`, target `(0, 1.2, 0)`, FOV 32°. Math: vertical extent at distance D with FOV 32° is `2·D·tan(16°)` ≈ `0.574·D`. At D=3.2 the extent is ≈1.83 units — exactly the 1.85-unit humanoid height with zero margin. Worse, look target Y=1.2 put the frame's bottom around Y=0.4, cutting legs (Y=0 to Y=0.65) entirely. Resolved at `/ce:work` time by pulling the camera back to Z=4.5 and lowering look target to Y=0.9 with unchanged FOV 32°; new extent ≈2.58 units with ~0.73 units of margin. **Rule for future spec consultations:** always pass the model's bounding-box dimensions in the prompt. The external agent's math is abstract without them and clipping bugs survive into the implementation. For a 1.85-unit humanoid with margin, D ≥ 4.0.
- **ESLint flat-config codemod output was wrong twice.** `npx @next/codemod@canary next-lint-to-eslint-cli .` generated (a) an import path missing `.js` that Node ESM rejected, and (b) a direct import from `eslint-config-next/core-web-vitals` that returned legacy `{ extends: [...] }`, not a flat-config array. `eslint-config-next/core-web-vitals.js` exports `module.exports = { extends: [...] }` — it is **not** a flat-config-compatible module. **`FlatCompat` from `@eslint/eslintrc` is required despite the codemod's attempt to skip it.** M1 COMPOUND predicted this; the codemod did not.
- **ESLint flat-config requires explicit `.next/` ignore globs.** `next lint` had implicit build-artifact exclusions; `eslint.config.mjs` does not. Without `ignores: ['.next/**', 'node_modules/**', 'out/**', 'build/**', 'next-env.d.ts']`, ESLint walks into webpack-compiled chunks and emits ~500 spurious warnings.
...
- **PlayerModel's first draft claimed three.js auto-disposes geometries on mesh unmount.** False for prop-passed geometries. R3F only owns declarative `<boxGeometry>` JSX children; a geometry passed via `geometry={...}` is caller-owned and leaks GPU memory on every variant toggle until disposed. 6-of-9 first-pass reviewers flagged. Fixed by `useEffect` cleanup in commit `0df09aa`. Full writeup at `docs/solutions/performance-issues/r3f-geometry-prop-disposal-2026-04-18.md`.
- **Base mesh transparency bug slipped past the first `/ce:review`.** Every mesh was constructed with `transparent={true}`, making base layers semi-transparent against the dark background ("ghostly" bleed-through). Caught only by human visual verification, not by any static check or reviewer. Fixed to `transparent={isOverlay}` in commit `343754c`. Lesson: a boolean literal for a rendering-critical material prop deserves more scrutiny than "it compiles" can give.

### Invariants discovered

- **Slim UV packing convention: contiguous-start at `x=40` (right arm) / `x=32` (left arm) with unused tail.** Verified face-by-face against skinview3d's `setSkinUVs(box, u, v, w, h, d)` source during `/ce:work` (~99% confidence). 72 UVs pinned in `lib/three/geometry.ts` — canonical for M3 (raycast paint), M4 (2D↔3D bridge), M5 (bucket-fill island map), M7 (thumbnails), M11 (OG images). **M3+ must not re-investigate; just import.**
- **Tailwind v4 `@theme` resolves at runtime via `:root` CSS custom properties, not build-time literal substitution.** Tailwind compiles `@theme` to `:root { --font-sans: var(--font-geist), Inter, ... }`; the `.font-sans` utility produces literal `font-family: var(--font-sans)` which cascades. When `next/font` sets `--font-geist` on `<body>`, the chain resolves normally. Any nested element inside `<body>` gets Geist through the cascade without build-time baking.
- **R3F geometry lifecycle:** declarative `<boxGeometry>` JSX children are R3F-owned (auto-disposed on mesh unmount). `<mesh geometry={...}>` prop-passed instances are **caller-owned** — must dispose via `useEffect` cleanup. Same rule applies to textures, render targets, and shader materials passed as props. **Status in M2:** fixed in `0df09aa`; PlayerModel's `useEffect([geometries])` calls `.dispose()` on each of the 12 BoxGeometries when `variant` changes or the component unmounts.
- **Minecraft overlay material pattern:** `transparent={isOverlay}`, `alphaTest={isOverlay ? 0.01 : 0}`, `depthWrite={!isOverlay}`, `renderOrder={isOverlay ? 1 : 0}`. This is the canonical overlay rendering setup. Base meshes are opaque (depth-writing, no alpha); overlays are transparent (alpha-tested at 0.01 to clip padding texels, depth-reading only). **M3-M7 must NOT regress any of these four props.** Note: `renderOrder` only affects sorting within the transparent queue; if a base mesh ever flips to `transparent={true}`, the sort inverts and the overlay renders behind the base.
- **Minecraft overlay geometry is intentionally +1 pixel larger than its UV on each axis.** Overlay boxes share the base's UV rectangle at original dimensions, so the texture stretches ~6% wider. Creates the "puffier outer shell" effect. Not a bug — dismissed by adversarial reviewer as ecosystem precedent (skinview3d does the same).
- **`IDLE_ORBIT_RADIUS` is derived from `CAMERA_POSITION[2]`, not hardcoded.** Second-pass review (`421c3d3`) replaced the dual-literal pattern with a single source of truth. Future camera distance changes automatically keep the orbit coherent; the class of drift bug is eliminated.
...
- **Slim variant world-space arm X-position is `0.34375`** (vs `0.375` classic). Derived as `body_half_width` (0.25) + `arm_half_width` (variant-dependent: 0.09375 slim, 0.125 classic) — arms touch the body. **This is NOT Minecraft's in-game shoulder offset** of ±0.3125 (which places arm *centers* outboard with small gaps). **P3 follow-up for M3+:** if hover math, raycast paint, or the M5 mirror tool needs the in-game offset, reconcile there. M2 uses the body-touching offset because the placeholder skin and T-pose don't make the gap visible.

### Gotchas for future milestones

- **DOM touches in React hooks must use `useEffect`, not `useMemo`.** Any `document.*`, `new Image()`, `navigator.*`, `window.*` access at render time breaks Next.js static prerender. Gate with `useEffect` even if it complicates the consumer (null-check + conditional render). The M2 plan's spec for `usePlaceholderTexture` specified `useMemo` and had to be refactored.
- **Async-resource hooks need a `cancelled` flag.** For patterns like `new Image()` + `img.onload` + `tex.dispose()` cleanup: a `let cancelled = false` checked in `onload`/`onerror`, and handlers nulled in cleanup, prevents rapid prop-toggle races where the old handler fires on a disposed resource. M3's `TextureManager` should extract this pattern (or a shared `useAsyncResource` hook).
- **Variant/mode toggle buttons need ARIA + `data-*`.** `aria-pressed` + `data-variant` + `data-testid` are additive; agents and tests can assert state without OCR. Added retroactively in `/ce:review` — bake them in from the start in M3+.
...
- **Global-state geometry reconstruction needs explicit disposal.** Today M2 disposes BoxGeometries in `useEffect` cleanup keyed on `[geometries]` (which re-runs on variant change). **If M3 lifts geometry construction into the Zustand store or a provider above `PlayerModel`, the disposal contract must move with it** — otherwise `PlayerModel` must keep a local cleanup hook that runs when the geometry reference changes. Forgetting this reintroduces the caller-owned-leak class of bugs the solutions doc captures.
- **Placeholder skin overlay color is semi-transparent cyan** (`rgba(0, 229, 255, 0.35)`). Visually correct under the overlay material pattern, but it creates a "ghostly" appearance on the unpainted default model because real Minecraft skins use opaque overlay textures. Don't mistake this for a rendering bug during M3-M6 testing. Real Microsoft minecraft-samples skin in M7 replaces the placeholder with proper opaque overlay layers.
...
- **Human visual verification is not optional.** Automated review missed both the base-transparency bug and the camera-crop bug. Every milestone whose acceptance includes a rendered output needs at least one Chrome-screenshot pass before `/ce:compound`.
...
- `lib/three/constants.ts` — camera (position `[0, 1.4, 4.5]`, look target `[0, 0.9, 0]`, FOV 32°), breathing (1.5 Hz / 0.01), orbit (±3°, 9s period, 500ms warm-up, radius **derived** from `CAMERA_POSITION[2]`), rim-light `0x7FD6FF` (pinned, unused until M3)
- `lib/three/geometry.ts` — `CLASSIC_UVS` + `SLIM_UVS` + `PlayerPart` union + `BoxUVs` type + `partDims(variant, part)` + `partPosition(variant, part)` + `mapBoxUVs(geo, uvs)` + `getUVs(variant)`. Arm X-position: 0.375 classic, 0.34375 slim (body-touching, not in-game offset)
- `lib/three/placeholder-skin.ts` — `createPlaceholderSkinDataURL(variant)` (replaced in M7)
- `lib/three/PlayerModel.tsx` — `'use client'`, 16-mesh humanoid, single coalesced `useFrame`, `useEffect` disposal on variant change, canonical overlay material pattern
...
- Coupled constants are **derived** from their source of truth, not duplicated (e.g., `IDLE_ORBIT_RADIUS = CAMERA_POSITION[2]`).
...
- **M3 budget:** keep Zustand store + 2D paint canvas + tools registry under **+15 kB** total to maintain the performance trajectory.
...
- This file's M2 "Invariants" section — the Tailwind `@theme` runtime chain, `PART_ORDER` exhaustiveness pattern, and overlay material pattern will be immediately reused by M3's tool palette and paint surface.
- `lib/three/constants.ts` inline comment — camera framing math + lesson about passing bounding-box dimensions to external spec consultations.
